...
function replayfun()
  composer.removeScene("replay7" )
  composer.gotoScene( "replay7")
end
...
--  transition.to( obj, {time=4000,y=60} )
  transition.to( obj, {time=timer,x=screenLeft+math.random(230, 260),y=screenTop+50  ,onComplete=flybackup} )
  randomtime()
end
...
function flybackdown(obj)
  transition.to( obj, {time=timer  ,x=screenLeft+100, onComplete=flybackup1 } )
...
--  print("flyback up there")
--  transition.to( obj, {time=4000,y=60} )
  transition.to( obj, {time=timer,x=screenLeft+420,y=screenBottom-65  ,onComplete=flybackdown} )
  randomtime()
end
...
--  transition.to( enemy1, {time=20000  ,rotation=180, onComplete=flybackup } )
  ...
  -- transition.to( enemy2, {delay=3000,time=4000  ,y=screenBottom-52, onComplete=flybackup } )
  transition.to( enemy1, {time=4000  ,x=screenLeft+250,y=screenTop+50, onComplete=flybackup } )
  ----------------------------------------------------------------------------
